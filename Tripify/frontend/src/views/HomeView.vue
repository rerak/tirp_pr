...
// API 호출 부분이 주석 처리되어 있습니다. 실제 사용 시 주석을 해제하고 경로를 맞춰주세요.
...
const loadFestivals = async () => {
  try {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      for (let i = 1; i <= 3; i++) {
        data.push({
          id: `${currentYear}${monthStr}${i}`,
          title: `${currentYear}년 ${m}월의 멋진 축제 ${i}`,
          region: ['서울', '부산', '제주', '강원'][i % 4],
          event_start_date: `${currentYear}${monthStr}01`
        })
      }
    }
